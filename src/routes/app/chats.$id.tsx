...
import {
  ...
  mergeDMLists,
  replaceLocalDM,
  updateChatsListCache,
} from "@/lib/dm-delivery";
...
function MessageTicks({ status, isFriend }: { status: MessageStatus; isFriend: boolean }) {
  if (status === "pending")
    return <Clock className="h-3 w-3 opacity-70" aria-label="قيد الإرسال" />;
  if (status === "sent") return <Check className="h-3 w-3 opacity-70" aria-label="تم الإرسال" />;
  if (status === "delivered")
    return <CheckCheck className="h-3 w-3 opacity-70" aria-label="تم التسليم" />;
...
function DMPage() {
  ...
  useEffect(() => {
    if (!user) return;
    const loadSeq = activeLoadSeqRef.current + 1;
    activeLoadSeqRef.current = loadSeq;
    (async () => {
      ...
      if (cachedMsgs) {
        console.info("[dm-cache] loaded-local", {
          key: dmKey,
          count: cachedMsgs.length,
          online: getOnline(),
        });
        setMessages((prev) => mergeDMLists(prev, cachedMsgs));
        latestMessageAtRef.current = Math.max(
          latestMessageAtRef.current,
          ...cachedMsgs.map((m) => Date.parse(m.created_at) || 0),
        );
      } else {
      ...
      try {
        const [
          ...
          supabase
            .from("profiles")
            .select("id, username, avatar_url, last_seen_at, hide_last_seen")
            .eq("id", otherId)
            .maybeSingle(),
          supabase
            .from("direct_messages")
            .select(
              "id, sender_id, receiver_id, content, created_at, message_type, media_url, media_duration_ms, reply_to_id, delivered_at, read_at",
            )
            .or(
        ...
        if (p) {
          ...
          await cacheSet(cacheKeys.profile(otherId), p as Profile);
        }
        ...
        if (loadSeq !== activeLoadSeqRef.current) {
          console.info("[dm-cache] stale-cloud-load-ignored", {
            key: dmKey,
            loadSeq,
            active: activeLoadSeqRef.current,
          });
          return;
        }
        const newestFreshAt = Math.max(0, ...fresh.map((m) => Date.parse(m.created_at) || 0));
        if (fresh.length && newestFreshAt + 1000 < latestMessageAtRef.current) {
          console.info("[dm-cache] stale-cloud-snapshot-merged-only", {
            key: dmKey,
            newestFreshAt,
            latestKnownAt: latestMessageAtRef.current,
          });
        }
        setMessages((prev) => {
          const next = mergeDMLists(prev, fresh);
          latestMessageAtRef.current = Math.max(
            latestMessageAtRef.current,
            ...next.map((m) => Date.parse(m.created_at) || 0),
          );
          return next;
        });
        setBlocked(!!bl);
  ...
  useEffect(() => {
    ...
    const onGlobalMessage = (event: Event) => {
      const detail = (event as CustomEvent<{ message?: DM; peerId?: string; replacedId?: string }>)
        .detail;
      const msg = detail?.message;
      if (!msg || detail?.peerId !== otherId) return;
      const msgTime = Date.parse(msg.created_at) || 0;
      if (!detail?.replacedId && msgTime + 1000 < latestMessageAtRef.current) {
        console.info("[dm-chat] stale-realtime-payload-merge-only", {
          messageId: msg.id,
          peerId: otherId,
          msgTime,
          latestKnownAt: latestMessageAtRef.current,
      ...
      setMessages((old) => {
        const next = mergeDMList(old, msg, detail?.replacedId);
        latestMessageAtRef.current = Math.max(
          latestMessageAtRef.current,
          ...next.map((m) => Date.parse(m.created_at) || 0),
        );
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data } = await supabase
        .from("calls")
        .select(
          "id, caller_id, callee_id, call_type, status, started_at, answered_at, ended_at, duration_seconds, end_reason",
        )
        .or(
          `and(caller_id.eq.${user.id},callee_id.eq.${otherId}),and(caller_id.eq.${otherId},callee_id.eq.${user.id})`,
        )
  ...
  const send = async (e: React.FormEvent) => {
    ...
    if (blocked) {
      toast.error("لا يمكن المراسلة، لقد قمت بحظر هذا المستخدم");
      return;
    }
    setSending(true);
    ...
    if (!getOnline()) {
      const queuedRow = await enqueueMessage({
        kind: "dm",
        sender_id: user.id,
        receiver_id: otherId,
        content,
    ...
    await appendLocalDM(user.id, optimistic);
    console.info("[dm-chat] optimistic-saved", {
      messageId: tempId,
      peerId: otherId,
      type: "text",
    });
    ...
    if (inserted) {
      ...
      console.info("[dm-chat] optimistic-confirmed", {
        tempId,
    ...
    if (error) {
      ...
      console.warn("[dm-chat] send-failed-optimistic-kept", {
        tempId,
        peerId: otherId,
        error: error.message,
      });
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      if (name === "NotAllowedError" || name === "SecurityError") {
        toast.error("صلاحية المايكروفون مطلوبة", {
          description: "فعّلها من إعدادات التطبيق لإرسال رسائل صوتية",
        });
  ...
  const shareMessage = async (m: DM) => {
    ...
    if (navigator.share) {
      ...
      try {
        await navigator.clipboard.writeText(txt);
        toast.success("تم النسخ للمشاركة");
      } catch {
        toast.error(t("common.error"));
      }
    }
    ...
    <main className="flex flex-col bg-background" style={{ height: "100dvh" }}>
      ...
      >
        ...
        <button
          ...
          aria-label="رجوع"
          className="relative p-1.5 rounded-full hover:bg-white/10 active:scale-95 transition"
        >
        ...
        >
          <StoryRing userId={other?.id} size="sm">
            {otherAvatarSource ? (
              <img
                src={otherAvatarSource}
                alt=""
                className="h-11 w-11 rounded-full object-cover ring-2 ring-white/40 shadow-lg transition-transform active:scale-95"
              />
            ) : (
        ...
        {other && !blocked && !blockedByOther && <CallButtons peer={other} />}
      ...
      {blockedByOther && (
        <div className="bg-destructive/10 text-destructive text-center text-xs py-2 px-3">
          قام هذا المستخدم بحظرك
        </div>
      )}
      {blocked && (
        <div className="bg-yellow-500/10 text-yellow-700 dark:text-yellow-400 text-center text-xs py-2 px-3">
        ...
        {messages.length === 0 ? (
          ...
          <ul className="flex flex-col gap-2.5">
            {(() => {
              ...
              const items: Item[] = [];
              messages.forEach((m) =>
                items.push({ kind: "msg", t: new Date(m.created_at).getTime(), m }),
              );
              calls
                ...
                .forEach((c) =>
                  items.push({
                    kind: "call",
                    t: new Date(c.ended_at ?? c.started_at).getTime(),
                    c,
                  }),
                );
              ...
              return items.map((it) => {
                ...
                return (
                  <li key={m.id} className={`flex ${mine ? "justify-end" : "justify-start"}`}>
                    <div
                      className={`flex items-end gap-1.5 max-w-[85%] ${mine ? "flex-row" : "flex-row-reverse"}`}
                    >
                      <div className="relative shrink-0 self-center">
                        ...
                        {menuFor === m.id && (
                          <>
                            ...
                            <div
                              className={`absolute z-50 mt-1 ${mine ? "left-0" : "right-0"} top-full w-48 overflow-hidden rounded-xl border border-border bg-card shadow-lg`}
                            >
                              <ActionItem
                      ...
                      <div className="min-w-0 flex-1">
                        <MessageBubble
                          ...
                          onPress={() => setMenuFor(m.id)}
                        />
                        <div
                          className={`mt-1 flex items-center gap-1 text-[10px] text-muted-foreground/80 ${mine ? "justify-end" : "justify-start"}`}
        ...
        <div className="border-t border-border bg-secondary/60 px-3 py-2 flex items-center gap-2">
          ...
          <div className="min-w-0 flex-1 text-xs">
            ...
            <div className="truncate text-muted-foreground">
              {replyTo.content || (replyTo.message_type === "image" ? "صورة" : "رسالة صوتية")}
            </div>
          </div>
      ...
      {pendingMedia ? (
        <div
          className="sticky bottom-0 z-20 flex flex-col gap-2 border-t border-border bg-card px-3 py-3"
        ...
        <div
          className="sticky bottom-0 z-20 flex items-center gap-3 border-t border-border bg-card px-3 py-3"
          style={{ paddingBottom: "calc(env(safe-area-inset-bottom) + 0.75rem)" }}
        >
          <span className="flex h-3 w-3 animate-pulse rounded-full bg-destructive" />
          <span className="flex-1 text-sm font-medium">
            جاري التسجيل… {formatTime(recordSeconds)}
          </span>
          <button
        ...
        <form
          onSubmit={send}
          className="sticky bottom-0 z-20 flex items-center gap-2 border-t border-border bg-card px-3 py-2.5"
        ...
        >
          ...
          <button
            ...
            disabled={uploading || blocked || blockedByOther}
            className="flex h-11 w-11 items-center justify-center rounded-full bg-secondary text-foreground disabled:opacity-50"
            aria-label="صورة"
          >
            {uploading ? (
          ...
          {text.trim() ? (
            <button
              type="submit"
              disabled={sending || blocked || blockedByOther}
            ...
            <div className="max-h-[70vh] divide-y divide-border overflow-y-auto">
              {calls.length === 0 ? (
                ...
                [...calls].reverse().map((c) => {
                  ...
                  let label = "";
                  if (c.status === "missed")
                    label = outgoing ? "لم يُرد على المكالمة" : "مكالمة فائتة";
                  else if (c.status === "rejected")
                    label = outgoing ? "تم رفض المكالمة" : "رفضت المكالمة";
                  ...
                  return (
                    <div key={c.id} className="flex items-center gap-3 px-4 py-3">
                      <div
                        className={`flex h-11 w-11 items-center justify-center rounded-full ${missed ? "bg-rose-500/10 text-rose-500" : outgoing ? "bg-emerald-500/10 text-emerald-600" : "bg-sky-500/10 text-sky-600"}`}
                      >
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1.5 text-sm font-semibold">
                          <DirIcon
                            className={`h-3.5 w-3.5 ${missed ? "text-rose-500" : "text-muted-foreground"}`}
                          />
...
function CallEvent({ c, mine }: { c: CallRow; mine: boolean }) {
  ...
  return (
    <div className="my-1 flex justify-center">
      <div
        className={`flex max-w-[90%] items-center gap-2.5 rounded-full border px-3 py-1.5 shadow-sm backdrop-blur ${tone}`}
      >
...
function ActionItem({
  ...
  return (
    <button
      onClick={onClick}
      className={`flex w-full items-center gap-3 px-4 py-2.5 text-sm hover:bg-secondary text-start ${destructive ? "text-destructive" : ""}`}
    >
      {icon} {label}
    </button>
  );
...
function MessageBubble({
  ...
  const replyBlock = replied && (
    <div
      className={`mb-1 rounded-lg border-s-2 px-2 py-1 text-[11px] ${mine ? "border-primary-foreground/60 bg-primary-foreground/10" : "border-primary bg-secondary"}`}
    >
      <div className="font-bold opacity-80">رد</div>
      <div className="truncate opacity-90">
        {replied.content || (replied.message_type === "image" ? "صورة" : "رسالة صوتية")}
      </div>
    </div>
  );
  ...
  if (shared) {
    return (
      <div className={base} onClick={onPress}>
        {replyBlock}
        <TrackDMPlayer track={shared.track} senderName={shared.senderName} mine={mine} />
      </div>
    );
  }
  return (
  ...
  return (
    <div className="flex items-center gap-2">
      <button
        type="button"
        onClick={(e) => toggle(e)}
        className={`flex h-8 w-8 items-center justify-center rounded-full ${mine ? "bg-primary-foreground/20" : "bg-secondary"}`}
      >
        {playing ? <Pause className="h-4 w-4" /> : <Play className="h-4 w-4" />}
      </button>
      <div className="flex h-1 w-32 items-center">
        <div
          className={`h-1 w-full rounded-full ${mine ? "bg-primary-foreground/30" : "bg-muted"}`}
        />
      </div>
