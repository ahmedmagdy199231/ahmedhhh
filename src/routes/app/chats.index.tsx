  ...
  if (source) {
    return (
      <img
        src={source}
        alt=""
        className="h-12 w-12 rounded-xl object-cover ring-1 ring-emerald-500/30"
      />
    );
  }
  return (
...
function ChatsPage() {
  ...
  const latestListAtRef = useRef(0);
  ...
  const load = async () => {
    ...
    if (cached) {
      cachedList = cached;
      console.info("[dm-cache] loaded-local-list", {
        key: cacheKeys.chatsList(user.id),
        count: cached.length,
        online: getOnline(),
      });
      setConvos((prev) => mergeDMConversations(prev, cached));
      setLoading(false);
      latestListAtRef.current = Math.max(
        latestListAtRef.current,
        ...cached.map((c) => Date.parse(c.created_at) || 0),
      );
      convoCacheReadyRef.current = true;
    } else {
      console.info("[dm-cache] miss-local-list", {
        key: cacheKeys.chatsList(user.id),
        online: getOnline(),
      });
      setLoading(true);
    ...
    try {
      ...
      const ids = Array.from(
        new Set([...cachedList.map((c) => c.otherId), ...Array.from(map.keys())]),
      );
      if (ids.length === 0) {
        convoCacheReadyRef.current = true;
        setConvos([]);
        await cacheSet(cacheKeys.chatsList(user.id), []);
        setLoading(false);
        return;
      }
      ...
      const out: Convo[] = ids
        .map((id) => {
          ...
          const useFreshLast =
            !!fresh && (!cachedConvo || fresh.created_at >= cachedConvo.created_at);
          return {
            otherId: id,
            ...
            created_at: useFreshLast
              ? fresh.created_at
              : (cachedConvo?.created_at ?? fresh?.created_at ?? new Date(0).toISOString()),
      ...
      if (loadSeq !== activeLoadSeqRef.current) {
        console.info("[dm-list] stale-cloud-list-ignored", {
          loadSeq,
      ...
      convoCacheReadyRef.current = true;
      setConvos((prev) => {
        const next = mergeDMConversations(prev, out);
        latestListAtRef.current = Math.max(
          latestListAtRef.current,
          ...next.map((c) => Date.parse(c.created_at) || 0),
        );
        return next;
      });
      console.info("[dm-cache] loaded-cloud-list", {
        key: cacheKeys.chatsList(user.id),
        count: out.length,
      });
      const latestCached = (await cacheGet<Convo[]>(cacheKeys.chatsList(user.id))) ?? cachedList;
      await cacheSet(cacheKeys.chatsList(user.id), mergeDMConversations(latestCached, out));
    } catch {
      // offline — keep cached
    } finally {
      setLoading(false);
    ...
    const onConversations = (event: Event) => {
      ...
      setConvos((prev) => {
        const incomingNewest = Math.max(
          0,
          ...detail.list!.map((c) => Date.parse(c.created_at) || 0),
        );
        if (incomingNewest + 1000 < latestListAtRef.current) {
          console.info("[dm-list] stale-realtime-list-merge-only", {
            incomingNewest,
            latestKnownAt: latestListAtRef.current,
          });
        }
        const next = mergeDMConversations(prev, detail.list!);
        latestListAtRef.current = Math.max(
          latestListAtRef.current,
          ...next.map((c) => Date.parse(c.created_at) || 0),
        );
      ...
      <header className="sticky top-0 z-10 overflow-hidden border-b border-emerald-500/15 bg-gradient-to-b from-slate-950/90 via-slate-950/80 to-slate-950/60 px-5 py-5 backdrop-blur-2xl">
        ...
        <div className="relative flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <h1 className="truncate text-2xl font-black text-white tracking-tight">
              {t("chats.title")}
            </h1>
      ...
      <div className="px-4 pt-4">
        <div className="relative group">
          ...
          {query && (
            <button
              onClick={() => setQuery("")}
              className="absolute top-1/2 left-3 -translate-y-1/2 text-emerald-400/70 hover:text-emerald-300"
            >
              <X className="h-4 w-4" />
            </button>
          )}
                    ...
                    >
                      {p.avatar_url ? (
                        <img
                          src={p.avatar_url}
                          alt=""
                          className="h-10 w-10 rounded-xl object-cover ring-1 ring-emerald-500/30"
                        />
                      ) : (
                      ...
                      <span className="rounded-full bg-gradient-to-br from-emerald-400 to-emerald-600 px-3 py-1 text-xs font-bold text-white shadow-[0_4px_12px_-2px_rgba(16,185,129,0.5)]">
                        دردشة
                      </span>
                    </button>
                  </li>
        ...
        {loading ? (
          ...
          <div className="mt-12 flex flex-col items-center text-center">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              ابحث عن مستخدم في الأعلى لبدء محادثة جديدة
            </p>
          </div>
        ) : (
          <ul className="flex flex-col gap-2.5">
            {convos.map((c) => {
              ...
              return (
                <li key={c.otherId}>
                  <ChatRow convo={c} unreadActive={unreadActive} />
                </li>
              );
            })}
          </ul>
...
function ChatRow({ convo, unreadActive }: { convo: Convo; unreadActive: boolean }) {
  ...
  const onClick = (e: React.MouseEvent) => {
    if (longPressedRef.current || menu) {
      e.preventDefault();
      e.stopPropagation();
      return;
    }
  ...
  return (
    <>
      ...
      >
        ...
        <StoryRing userId={convo.otherId} size="sm">
          <CachedAvatar url={convo.avatar_url} username={convo.username} />
        </StoryRing>
        <div className="min-w-0 flex-1">
          ...
          <div
            className={`truncate text-sm ${unreadActive ? "text-emerald-200/90" : "text-muted-foreground"}`}
          >
            {convo.last}
          </div>
        </div>
      ...
      {menu && (
        ...
        >
          <div
            className="w-full max-w-xs rounded-2xl bg-slate-900 border border-white/10 p-2"
