...
async function warmProfile(userId: string) {
  const { data } = await supabase
    .from("profiles")
    .select(
      "id, username, avatar_url, bio, last_seen_at, created_at, points, gender, country, hide_last_seen, dm_locked, profile_views, equipped_badge, equipped_name_color, equipped_chat_color, equipped_effect, is_banned, is_bot, game_wins",
    )
    .eq("id", userId)
    .maybeSingle();
...
async function warmRooms() {
  ...
  const { data: counts } = await supabase.from("room_members").select("room_id");
...
async function warmChatsAndRecentMessages(userId: string) {
  ...
  if (!ids.length) {
    console.info("[data-prewarm] chats-sync-empty-kept-cache", { userId });
    return;
  }
  const { data: profs } = await supabase
    .from("profiles")
    ...
    .map((id) => {
      ...
      return {
        ...
        created_at: useFreshLast
          ? fresh.created_at
          : (cached?.created_at ?? fresh?.created_at ?? new Date(0).toISOString()),
        unread: Math.max(fresh?.unread ?? 0, cached?.unread ?? 0),
      };
    })
    .filter((c) => c.last)
    .sort((a, b) => b.created_at.localeCompare(a.created_at));
  let mergedOut = mergeDMConversations(cachedList, out);
  await withCacheLock(cacheKeys.chatsList(userId), async () => {
    const latest = (await cacheGet<DMConversation[]>(cacheKeys.chatsList(userId))) ?? cachedList;
    mergedOut = mergeDMConversations(latest, out);
    await cacheSet(cacheKeys.chatsList(userId), mergedOut);
  });

  // Pre-cache last N messages for the top recent peers + their profile.
  const topPeers = mergedOut
    .slice()
      ...
      try {
        ...
        if (dm) {
          ...
          await withCacheLock(key, async () => {
            ...
            await cacheSet(key, mergeDMLists(local, freshRows.slice().reverse() as DMRow[]));
          });
          await Promise.all(
            (freshRows as CachedMessageMedia[]).map((m) => cacheMedia(m.media_url)),
          );
        }
        if (prof) {
