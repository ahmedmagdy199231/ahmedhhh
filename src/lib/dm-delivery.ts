...
const activeDeliveryWorkers = new Set<string>();

function dmTime(row: Pick<DMRow, "created_at">): number {
  const time = Date.parse(row.created_at);
  return Number.isFinite(time) ? time : 0;
}

export function sortDMRows<T extends DMRow>(rows: T[]): T[] {
  return rows.slice().sort((a, b) => {
    const byTime = dmTime(a) - dmTime(b);
    return byTime !== 0 ? byTime : a.id.localeCompare(b.id);
  });
}

export function mergeDMRow(existing: DMRow | undefined, incoming: DMRow): DMRow {
  if (!existing) return incoming;
  const existingTime = dmTime(existing);
  const incomingTime = dmTime(incoming);
  if (incomingTime < existingTime) {
    return {
      ...incoming,
      ...existing,
      delivered_at: existing.delivered_at ?? incoming.delivered_at ?? null,
...
export function mergeDMList<T extends DMRow>(list: T[], msg: T, replacedId?: string): T[] {
  const base = replacedId ? list.filter((m) => m.id !== replacedId) : list;
  const index = base.findIndex((m) => m.id === msg.id);
  if (index < 0) return sortDMRows([...base, msg]);
  const next = [...base];
  next[index] = mergeDMRow(next[index], msg) as T;
  return sortDMRows(next);
}

export function mergeDMLists<T extends DMRow>(current: T[], incoming: T[]): T[] {
  const byId = new Map<string, T>();
  current.forEach((m) => byId.set(m.id, m));
  incoming.forEach((m) => byId.set(m.id, mergeDMRow(byId.get(m.id), m) as T));
  return sortDMRows(Array.from(byId.values()));
}
...
export function mergeDMConversations(
  ...
  current.forEach((c) => byPeer.set(c.otherId, c));
  incoming.forEach((next) => {
    const prev = byPeer.get(next.otherId);
    if (!prev) {
      byPeer.set(next.otherId, next);
      return;
    }
    const useIncomingLast = dmTime(next as unknown as DMRow) >= dmTime(prev as unknown as DMRow);
    byPeer.set(next.otherId, {
      otherId: next.otherId,
      username: next.username || prev.username,
      avatar_url: next.avatar_url ?? prev.avatar_url,
      last: useIncomingLast ? next.last : prev.last,
      created_at: useIncomingLast ? next.created_at : prev.created_at,
      unread: Math.max(prev.unread ?? 0, next.unread ?? 0),
    });
  });
  return Array.from(byPeer.values()).sort(
    (a, b) => dmTime(b as unknown as DMRow) - dmTime(a as unknown as DMRow),
  );
}
...
export async function withCacheLock(key: string, task: () => Promise<void>): Promise<void> {
...
async function upsertLocalConversation(
  ...
  await withCacheLock(listKey, async () => {
    ...
    const shouldReplaceLast = !existing || dmTime(msg) >= dmTime(existing as unknown as DMRow);
    if (existing && !shouldReplaceLast) {
      console.info("[dm-global] stale-conversation-payload-ignored-for-last", {
        peerId,
        messageId: msg.id,
        messageAt: msg.created_at,
        currentAt: existing.created_at,
      });
    }
    updated = {
      otherId: peerId,
      ...
      unread:
        isIncomingUnread && options.incrementUnread
          ? (existing?.unread ?? 0) + 1
          : (existing?.unread ?? 0),
    };
  ...
  emitDmEvent(DM_CONVERSATIONS_EVENT, {
    list: nextList,
...
export async function replaceLocalDM(
  myUserId: string,
  peerId: string,
  tempId: string,
  msg: DMRow,
  ...
  try {
    await withCacheLock(key, async () => {
      ...
      if (!temp && real) {
        saved = mergeDMRow(real, msg);
        const next = list.map((m) => (m.id === msg.id ? saved : m));
        await cacheSet(key, sortDMRows(next));
        console.info("[dm-global] replace-local-already-merged", {
          key,
          tempId,
          messageId: msg.id,
        });
      ...
      saved = {
        ...(real ?? {}),
        ...msg,
        created_at: temp?.created_at ?? real?.created_at ?? msg.created_at,
      } as DMRow;
    ...
    console.info("[dm-global] replaced-local-message", {
      key,
      tempId,
      messageId: saved.id,
      stableCreatedAt: saved.created_at,
    });
    ...
    console.error("[dm-global] replace-local-message-failed", {
      key,
      tempId,
      messageId: msg.id,
      error,
...
export async function removeLocalDM(
  myUserId: string,
  peerId: string,
  messageId: string,
): Promise<void> {
...
export async function appendLocalDM(myUserId: string, msg: DMRow): Promise<boolean> {
  ...
  try {
    await withCacheLock(key, async () => {
      ...
      if (index >= 0) {
        const copy = [...list];
        copy[index] = mergeDMRow(copy[index], msg);
        saved = copy[index];
      ...
      const optimisticIndex = list.findIndex(
        (x) =>
          ...
          Math.abs(new Date(msg.created_at).getTime() - new Date(x.created_at).getTime()) <
            5 * 60 * 1000,
      );
      ...
      wasNew = true;
      const newestCachedAt = list.reduce((max, row) => Math.max(max, dmTime(row)), 0);
      if (dmTime(msg) + 1000 < newestCachedAt) {
        console.info("[dm-global] older-message-merged-without-overwrite", {
          key,
          messageId: msg.id,
          messageAt: msg.created_at,
          newestCachedAt,
    ...
    console.info("[dm-global] saved-local-message", {
      key,
      messageId: saved.id,
      peerId,
      wasNew,
...
export async function markLocalDelivered(
  myUserId: string,
  peerId: string,
  messageId: string,
): Promise<void> {
...
export function useDmDeliveryWorker(): void {
  ...
  const seenRealtimeInsertsRef = useRef<Set<string>>(new Set());
  ...
  useEffect(() => {
    ...
    const drainPending = async () => {
      if (drainInFlightRef.current) {
        console.info("[dm-global] drain-pending-skipped-in-flight", { userId: myId });
        return;
      }
    ...
    if (activeDeliveryWorkers.has(myId)) {
      console.warn("[dm-global] duplicate-worker-blocked", { userId: myId });
      return;
    }
    activeDeliveryWorkers.add(myId);
    ...
    const ch = supabase
      ...
      .on(
        ...
        async (p) => {
          ...
          if (seenRealtimeInsertsRef.current.has(m.id)) {
            console.info("[dm-global] duplicate-realtime-insert-ignored", { messageId: m.id });
            return;
          }
          seenRealtimeInsertsRef.current.add(m.id);
          console.info("[dm-global] realtime-insert", {
            messageId: m.id,
      ...
      .on(
        ...
        async (p) => {
          const m = p.new as DMRow;
          if (seenRealtimeInsertsRef.current.has(m.id)) {
            console.info("[dm-global] duplicate-realtime-outgoing-insert-ignored", {
              messageId: m.id,
            });
            return;
          }
          seenRealtimeInsertsRef.current.add(m.id);
          console.info("[dm-global] realtime-outgoing-insert", {
            messageId: m.id,
      ...
      .on(
        ...
        async (p) => {
          const m = p.new as DMRow;
          console.info("[dm-global] realtime-outgoing-update", {
            messageId: m.id,
            receiverId: m.receiver_id,
          });
          await appendLocalDM(myId, m);
      ...
      .on(
        ...
        async (p) => {
          const m = p.new as DMRow;
          console.info("[dm-global] realtime-incoming-update", {
            messageId: m.id,
            senderId: m.sender_id,
          });
          await appendLocalDM(myId, m);
      ...
      .on(
        ...
        async (p) => {
          ...
          if (!old?.id) return;
          console.info("[dm-global] realtime-outgoing-delete-kept", {
            messageId: old.id,
            receiverId: old.receiver_id,
          });
          await appendLocalDM(myId, {
            ...old,
            delivered_at: old.delivered_at ?? new Date().toISOString(),
          });
        },
      )
      .subscribe((status) => {
    ...
    return () => {
      ...
      activeDeliveryWorkers.delete(myId);
